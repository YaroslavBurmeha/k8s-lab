	...
	stages {
		...
		stage("deploy on k8s"){
			...
			steps{
				...
				sh 'curl -X POST "https://api.telegram.org/bot6991480940:AAGM38za6G7f6d8gZT5jirGRBZe2ICeXjL0/sendMessage" -d "chat_id=1341857329&text=билд под нмоером:$BUILD_NUMBER был успешно задеплоен в кластер kubernetes желаем вам удачи!"'
			}
		}
	}
}
